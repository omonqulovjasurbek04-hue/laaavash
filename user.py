...
from data import (
    ...
    savat_tozalash, buyurtma_bekor_qilish, malumot_yuklash, foydalanuvchilar_fayli,
    savatdan_ochirish
)
...
async def menyu_buyruq(message: Message):
    ...
    if matn == Menyular[0]:
        ...
        for savat_item_kalit, mahsulot in savat.items():
            ...
            inline_tugmalar.append([InlineKeyboardButton(text=f"❌ {mahsulot['name']} ni o'chirish", callback_data=f"remove_from_cart:{savat_item_kalit}")])

        savat_matni += f"💰 Jami: {jami} so'm"

        inline_kb = InlineKeyboardMarkup(inline_keyboard=inline_tugmalar)

        await message.answer("Savat menyusi:", reply_markup=ReplyKeyboardMarkup(keyboard=klaviatura, resize_keyboard=True))
        await message.answer(savat_matni, reply_markup=inline_kb)
...
async def buyurtma_berish(message: Message):
    ...
    if buyurtma:
        ...
        # Adminga yuborish
        foydalanuvchi_malumot = malumot_yuklash(foydalanuvchilar_fayli)
        adminlar = foydalanuvchi_malumot.get("adminlar", [])
        
        foydalanuvchi = next((u for u in foydalanuvchi_malumot.get("foydalanuvchilar", []) if u["id"] == message.from_user.id), None)
        telefon = foydalanuvchi.get("telefon", "Noma'lum") if foydalanuvchi else "Noma'lum"
        
        admin_xabar = f"🚨 Yangi buyurtma keldi!\n\n"
        admin_xabar += f"🆔 Buyurtma #{buyurtma['id']}\n"
        admin_xabar += f"👤 Mijoz: {message.from_user.full_name}\n"
        admin_xabar += f"📱 Telefon: {telefon}\n"
        admin_xabar += f"💰 Jami: {buyurtma['total']} so'm\n\n"
        admin_xabar += "Tarkibi:\n"
        for item in buyurtma['items'].values():
            admin_xabar += f"- {item['name']} ({item['quantity']} dona)\n"
            
        javob_klav = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(text="✅ Qabul qilish", callback_data=f"admin_accept_order:{buyurtma['id']}"),
                    InlineKeyboardButton(text="❌ Bekor qilish", callback_data=f"admin_decline_order:{buyurtma['id']}")
                ]
            ]
        )
        for adm in adminlar:
            try:
                await message.bot.send_message(adm, admin_xabar, reply_markup=javob_klav)
            except Exception as e:
                logger.error(f"Adminga xabar ketmadi: {e}")
    else:
        await message.answer("❌ Buyurtma yaratishda xatolik!")

@rr.callback_query(F.data.startswith("remove_from_cart:"))
async def savatdan_olib_tashlash(callback: CallbackQuery):
    try:
        _, kalit_str = callback.data.split(":", 1)
        kategoriya, mahsulot_kaliti = kalit_str.split(":")
        
        if savatdan_ochirish(callback.from_user.id, kategoriya, mahsulot_kaliti):
            await callback.answer("✅ Mahsulot savatdan olib tashlandi!")
            await callback.message.delete()
        else:
            await callback.answer("❌ Xatolik yuz berdi!")
    except Exception as e:
        logger.error(f"Savat ochirishda xatolik: {e}")
        await callback.answer("❌ Xatolik yuz berdi!")
