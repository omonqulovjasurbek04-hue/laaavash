...
async def admin_buyurtma_qabul_callback(callback: CallbackQuery):
    ...
    if buyurtma_holat_yangilash(buyurtma_id, "accepted"):
        ...
        javob_klav = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(text="🍳 Tayyorlashni boshlash", callback_data=f"admin_process_order:{buyurtma_id}"),
                    InlineKeyboardButton(text="❌ Bekor qilish", callback_data=f"admin_decline_order:{buyurtma_id}")
                ]
            ]
        )
        await callback.message.edit_text(admin_buyurtma_xulosa(buyurtma), reply_markup=javob_klav)
...
@rr.callback_query(F.data.startswith("admin_process_order:"))
async def admin_process_order_callback(callback: CallbackQuery):
    if not admin_tekshir(callback.from_user.id):
        await callback.answer("❌ Siz admin emassiz!", show_alert=True)
        return
    ...
    if buyurtma["status"] == "cancelled" or buyurtma["status"] == "completed":
        return await callback.answer("ℹ️ Bu holatini o'zgartirib bo'lmaydi.", show_alert=True)
        
    if buyurtma_holat_yangilash(buyurtma_id, "processing"):
        buyurtma["status"] = "processing"
        await callback.answer("⏳ Buyurtma tayyorlanmoqda.")
        javob_klav = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="🚗 Yetkazildi (Yakunlash)", callback_data=f"admin_complete_order:{buyurtma_id}")]
            ]
        )
        await callback.message.edit_text(admin_buyurtma_xulosa(buyurtma), reply_markup=javob_klav)
        
        bot = callback.message.bot if callback.message else None
        await foydalanuvchiga_xabar(bot, buyurtma, "⏳ Buyurtmangiz tayyorlanmoqda...", "Oshpazlarimiz taomingizni tayyorlashni boshlashdi.")
    else:
        await callback.answer("❌ Xatolik yuz berdi.", show_alert=True)


@rr.callback_query(F.data.startswith("admin_complete_order:"))
async def admin_complete_order_callback(callback: CallbackQuery):
    if not admin_tekshir(callback.from_user.id):
        await callback.answer("❌ Siz admin emassiz!", show_alert=True)
        return
    ...
    if buyurtma_holat_yangilash(buyurtma_id, "completed"):
        buyurtma["status"] = "completed"
        await callback.answer("✅ Buyurtma yakunlandi.")
        await callback.message.edit_text(admin_buyurtma_xulosa(buyurtma))
        
        bot = callback.message.bot if callback.message else None
        await foydalanuvchiga_xabar(bot, buyurtma, "🎉 Buyurtmangiz yetkazildi!", "Yoqimli ishtaha. Bizni tanlaganingiz uchun rahmat!")
    else:
        await callback.answer("❌ Xatolik yuz berdi.", show_alert=True)
